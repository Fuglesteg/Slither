    ...
    (let (behavior-symbols methods)
      ...
      `(progn
         ...
         (defmethod entity-encode ((entity ,name))
           (let ((entity-data (map '(vector (unsigned-byte 8))
                                   #'slither/networking::encode-argument
                                   (list ,@(loop for slot in slot-symbols
                                                 collect `(slot-value entity ',slot))))))
           (concatenate
            ...
            ,@(loop for behavior-symbol in behavior-symbols
                    collect `(slither/behaviors::behavior-encode (entity-find-behavior entity ',behavior-symbol))))))
         (defmethod entity-decode ((entity-type-id (eql ,entity-type-id)) entity-vector)
           (let* ((entity-size (vector-read-integer entity-vector :bytes 2))
                  (arguments (when (< 0 entity-size)
                               (slither/networking::decode-arguments (subseq entity-vector 2 entity-size))))
                  (index (+ 2 entity-size))
             ...
             (setf (entity-behaviors entity)
                   (list ,@(loop for behavior-symbol in behavior-symbols
                                 collect `(multiple-value-bind (behavior behavior-size)
                                              (slither/behaviors::behavior-decode ',behavior-symbol (subseq entity-vector index) entity)
                                            (incf index (+ 2 behavior-size))
                                            behavior))))))
         ,@methods)))))
