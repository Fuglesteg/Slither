...
(defconstant +max-past-tick-amount+ 10)
...
(defun (setf client-connection-entities) (new-entities client-connection)
  ...
  (dolist (entity new-entities)
    (setf (networked-connection) client-connection))
      ...
      (let ((wake-time (+ last-time (tick-delta))))
        ...
        (let ((slither/core::*tick* tick)
          ...
          (when *lag-compensate-from-tick*
            (dolist (networked (networked-objects))
              (networked-rewind-to-tick networked tick))
            (dotimes (tick (- tick *lag-compensate-from-tick*))
              (dolist (networked (networked-objects))
