...
(defgeneric pre-tick (entity)
  ...
  (:method :before ((entity entity))
    (loop for behavior in (entity-behaviors entity)
          do (pre-tick behavior))))

(defgeneric post-tick (entity)
  (:method ((entity entity)))
  (:method :around ((entity entity))
    (let ((*entity* entity))
      (call-next-method)))
...
(defgeneric pre-fixed-tick (entity)
  (:method ((entity entity)))
  (:method :around ((entity entity))
    (let ((*entity* entity))
      (call-next-method)))
  (:method :before ((entity entity))
    (loop for behavior in (entity-behaviors entity)
          do (pre-fixed-tick behavior))))

(defgeneric post-fixed-tick (entity)
  (:method ((entity entity)))
  (:method :around ((entity entity))
    (let ((*entity* entity))
      (call-next-method)))
  (:method :before ((entity entity))
    (loop for behavior in (entity-behaviors entity)
          do (post-fixed-tick behavior))))

(defgeneric fixed-tick (entity)
  (:method ((entity entity)))
  (:method :around ((entity entity))
    (let ((*entity* entity))
      (pre-fixed-tick entity)
      (call-next-method)
      (post-fixed-tick entity)))
  (:method :before ((entity entity))
    (loop for behavior in (entity-behaviors entity)
          do (fixed-tick behavior))))

(defgeneric pre-start (entity)
  (:method ((entity entity)))
  (:method :around ((entity entity))
    (let ((*entity* entity))
      (call-next-method)))
  (:method :before ((entity entity))
    (loop for behavior in (entity-behaviors entity)
          do (pre-start behavior))))

(defgeneric post-start (entity)
  (:method ((entity entity)))
  (:method :around ((entity entity))
    (let ((*entity* entity))
      (call-next-method)))
                         ...
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod tick ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                        ((string= keyword :pre-tick)
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod pre-tick ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                        ((string= keyword :post-tick)
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod post-tick ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                         ...
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod fixed-tick ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                        ((string= keyword :pre-fixed-tick)
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod pre-fixed-tick ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                        ((string= keyword :post-fixed-tick)
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod post-fixed-tick ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                         ...
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod start ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                        ((string= keyword :pre-start)
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod pre-start ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
                        ((string= keyword :post-start)
                         (push
                          (let ((entity-symbol (gensym)))
                            `(defmethod post-start ((,entity-symbol ,name))
                               ,@arguments))
                          methods))
